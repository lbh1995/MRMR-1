...
def SelectOnNB(featuresSets):
	model = BernoulliNB()
	model.fit(X_train[:,featuresSets], y_train)
	y_Pred = model.predict(X_test[:,featuresSets])
	score = evaluate_binary_predictions(y_test,y_Pred)
	outputTest.write("times = %d, selectMethod = %d, selectedAUC = %f, selectedMCC = %f, selectedACC = %f\n"%(i, selectMethod, score[0], score[1], score[2]))
	return score

def SelectOnLR(featuresSets):
	model = LogisticRegression()
	model.fit(X_train[:,featuresSets], y_train)
	y_Pred = model.predict(X_test[:,featuresSets])
	score = evaluate_binary_predictions(y_test,y_Pred)
...
#print PCCij
outputTest = open("RFSLoutputTest.txt", "w")
...
#selectAndTrainSelectOne(NSize, 2)
#selectAndTrainSelectOneLR(NSize, 1)
#clf = RandomForestClassifier(n_estimators=10)
#selectAndTrainSelectOneRF(NSize, 1)
FSModel = RandomForestClassifier(n_estimators=500, random_state=0)
FSModel.fit(X_train, y_train)
importances = FSModel.feature_importances_
indices = np.argsort(importances)[::-1]
featuresSet = []
for f in range(500):
	featuresSet.append(indices[f])
scores0 = []
scores1 = []
scores2 = []
for i in range(10, NSize+1, 10):
#		print i
	featuresSets = featuresSet[0:i]
	score = SelectOnNB(featuresSets)
#	print featuresSets
...
#		print i
	featuresSets = featuresSet[0:i]		
	score = SelectOnLR(featuresSets)	#print featuresSets
	scores0.append(score[0])
	scores1.append(score[1])
	scores2.append(score[2])
outputTest.write("AUC = %s "%scores0)
